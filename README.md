...
- 8종 업적과 진행도, 달성 날짜 표시
- 새 업적 달성 시 결과 화면 알림
- 업적 진행으로 해금하는 Classic, Midnight, Neon 테마
- 앱 재실행 후에도 유지되는 테마 선택
- 기기 언어에 맞춘 영어, 한국어, 일본어 UI
- 난이도별 플레이 횟수, 최고 기록, 최근 10게임 평균
- 오답·정확도 통계와 최근 기록 추이
- DAILY, streak, 업적 요약 통계
- 게임 완료 기록 저장
- 게임 완료 결과 화면 표시
...
lib/
  ...
  l10n/
    app_en.arb                      # 영어 기본 및 fallback 문구
    app_ko.arb                      # 한국어 문구
    app_ja.arb                      # 일본어 문구
    domain_localizations.dart       # 도메인 enum과 결과 표시 번역
    generated/                      # gen_l10n 생성 코드
  models/
    achievement.dart                # 업적 정의와 목표
    achievement_progress.dart       # 업적 진행도와 달성 상태
    app_theme_id.dart               # 테마 식별자와 업적 해금 조건
  ...
  services/
    ...
    theme_preference_storage.dart   # 선택 테마 저장/로드
  theme/
    game_theme.dart                 # semantic 색상과 ThemeData
  widgets/
    ad_mob_banner.dart              # AdMob 배너 위젯
    game_result_dialog.dart         # 게임 완료 결과 다이얼로그
    number_grid.dart                # 5x5 숫자 버튼 그리드
    theme_picker_sheet.dart         # 테마 미리보기와 잠금 상태 선택 UI
...
새 Classic 기록은 오답과 총 탭 수를 포함합니다.

```text
difficulty::record::date::wrongTapCount::totalTapCount
```

기존 2필드/3필드 기록은 계속 시간 통계에 포함되지만, 저장된 탭 정보가
없으므로 정확도와 오답 평균에서는 제외합니다.
...
테마는 메인 화면의 팔레트 버튼에서 선택합니다. Classic은 기본 제공,
Midnight는 업적 3개 달성, Neon은 7 Day Streak 업적 달성 시 해금됩니다.
선택 값은 `selected_theme` key에 저장하며, 저장된 테마가 잠긴 상태이면
Classic으로 안전하게 복구합니다.

앱은 기기 언어가 한국어면 한국어, 일본어면 일본어, 그 외에는 영어로
표시합니다. 사용자 문구는 `lib/l10n/app_*.arb`에서 관리하고
`flutter gen-l10n`으로 코드를 생성합니다. `1to25`, SharedPreferences
key, enum 저장 값, 기록 직렬화 포맷은 번역하지 않습니다.

## 주요 의존성

- `shared_preferences`: 기록 저장
- `intl`: 날짜 포맷
- `flutter_localizations`: 기기 locale과 Material 다국어 지원
